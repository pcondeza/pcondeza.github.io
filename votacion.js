...
$(document).ready(function() {
    ...
    $.each(regionesYComunas, function(region) {
        $("#region").append(new Option(region, region));
    });

    // Actualizar el select de comunas cuando se seleccione una región
    $("#region").change(function() {
        const regionSeleccionada = $(this).val();
        const comunas = regionesYComunas[regionSeleccionada];
        
        $("#comuna").empty(); // Limpiar el select de comunas
...
function enviarFormulario() {
    ...
    if (!validarRUT(rut)) {
        alert("RUT inválido");
        return false; // No limpiar el formulario
    }
    ...
    var formData = $("#formulario-votacion").serialize();

    $.ajax({
        url: 'procesar_votacion.php',
        type: 'POST',
        data: formData,
        success: function(response) {
            alert("Votación registrada exitosamente");
            $("#formulario-votacion")[0].reset(); // Limpiar el formulario solo en caso de éxito
        },
        error: function() {
            alert("Ocurrió un error al procesar la votación");
        }
    });
